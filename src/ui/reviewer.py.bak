...
# Patched: improved source matching and Excel/CSV preview support.
...
# Improved guess_source_file
def guess_source_file(json_path):
    """
    Heuristics to locate the original source file for a given parsed JSON.
    Strategies (in order):
      1) direct stem containment: if mock file stem contains the json stem -> match
      2) token match: split json stem into tokens (client, a, output) and match meaningful tokens (e.g. 'client_a' or 'a')
      3) prefix match: match client identifier like 'client_a' in file name
      4) fallback: return first PDF/image/Excel in mock folder if nothing else
    """
    name = json_path.stem  # e.g., client_a_output
    tokens = [t for t in name.split('_') if t]  # e.g., ['client','a','output']
    candidate_tokens = []
    if len(tokens) >= 2:
        candidate_tokens.append(f"{tokens[0]}_{tokens[1]}")  # client_a
    for t in tokens:
        if t.lower() != 'client' and t not in candidate_tokens:
    ...
    for d in CANDIDATE_MOCKS:
        if not d.exists():
            continue
        # 1) direct stem containment (preferred)
        for f in d.iterdir():
            if not f.is_file():
                continue
        ...
        for tok in candidate_tokens:
            for f in d.iterdir():
                if not f.is_file():
                    continue
                if tok.lower() in f.stem.lower():
                    return f
        # 3) try prefix match
        for f in d.iterdir():
            if not f.is_file():
                continue
            if f.stem.lower().startswith(name.lower().split('_')[0]):
                return f

    # Final fallback: return first pdf/image/excel/csv in the first existing mock dir
    for d in CANDIDATE_MOCKS:
        if not d.exists():
            continue
        for f in d.iterdir():
...
with col2:
    ...
    if st.button("Save corrections"):
        ...
        # append audit log (append mode)
        try:
            audit_f = labels_dir / "audit.log"
            audit_entry = {
                'json': str(sel.name),
                'saved_to': str(outp),
            ...
            with open(audit_f, "a", encoding="utf-8") as af:
